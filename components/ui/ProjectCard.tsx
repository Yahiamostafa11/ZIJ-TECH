...
function ProjectPreview({ type }: { type: ProjectCardProps["type"] }) {
  if (type === "automation") {
    return (
      <div className="relative h-full w-full bg-[radial-gradient(circle_at_50%_40%,rgba(242,193,102,0.15),transparent_45%),linear-gradient(135deg,rgb(var(--bg-card)),rgb(var(--bg-primary)))]">
  ...
  if (type === "analytics") {
    return (
      <div className="flex h-full items-end gap-3 bg-[linear-gradient(180deg,rgb(var(--bg-card)),rgb(var(--bg-primary)))] px-8 pb-8">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(var(--grid-line)_1px,transparent_1px)] bg-[length:100%_34px]" />
      </div>
    );
  }

  return (
    <div className="h-full bg-[linear-gradient(135deg,rgb(var(--bg-card)),rgb(var(--bg-primary)))] p-5">
...
export function ProjectCard({ title, description, type }: ProjectCardProps) {
  return (
    <div className="group overflow-hidden rounded-lg border border-border-subtle bg-bg-card/78 shadow-card transition duration-300 hover:-translate-y-1 hover:border-gold-light/60 hover:shadow-[0_0_32px_rgba(242,193,102,0.14)]">
